...
const getInitialState = (): ThemeState => {
  ...
  // Default to dark mode if no saved theme preference
  let isDarkMode = savedTheme !== "light"; // Dark by default, unless explicitly set to light
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  // useLayoutEffect ensures the class is updated BEFORE visual paint and before child effects run
  // This prevents race conditions where children read CSS variables before the class change
  React.useLayoutEffect(() => {
    ...
    localStorage.setItem(
      "portfolio-time-ratio",
      themeState.timeRatio.toString(),
    );
    localStorage.setItem(
      "portfolio-auto-mode",
      themeState.isAutoMode.toString(),
    );
  }, [themeState]);

  const toggleTheme = useCallback(() => dispatch({ type: "TOGGLE_THEME" }), []);
  const setTheme = useCallback(
    (isDark: boolean) => dispatch({ type: "SET_THEME", payload: isDark }),
    [],
  );
  const setCurrentTheme = useCallback(
    (theme: string) => dispatch({ type: "SET_CURRENT_THEME", payload: theme }),
    [],
  );
  const setTimeRatio = useCallback(
    (ratio: number) => dispatch({ type: "SET_TIME_RATIO", payload: ratio }),
    [],
  );
  const setIsAutoMode = useCallback(
    (auto: boolean) => dispatch({ type: "SET_IS_AUTO_MODE", payload: auto }),
    [],
  );
  const updateThemeFromTime = useCallback(
    (ratio: number) =>
      dispatch({ type: "UPDATE_THEME_FROM_TIME", payload: ratio }),
    [],
  );
